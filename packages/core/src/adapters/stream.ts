...
/**
 * Observability layer using streamEvents().
 *
 * Runs in parallel alongside .stream() when tracing is enabled.
 * Does NOT produce the response — only captures lifecycle events
 * for LangSmith tracing, latency metrics, and retry visibility.
 *
 * Call this alongside streamModel() when config.tracing.enabled is true.
 */
export async function observeStream(
  ...
  callbacks?: {
    ...
    onModelEnd?: (usage: TokenUsage) => void;
  }
): Promise<void> {
  // Only run if client supports streamEvents (not always available on bound models)
  if (!client.streamEvents) return;

  try {
    const stream = client.streamEvents(messages, { version: "v2" });
    for await (const event of stream) {
      switch (event.event) {
        ...
        case "on_retry":
          callbacks?.onRetry?.(typeof event.data === "string" ? event.data : "unknown");
          break;
      }
    }
  } catch {
    // Observability is best-effort — never crash the main flow
  }
}
