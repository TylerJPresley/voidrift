...
describe("Config Loader", () => {
  it("loads a valid global config", () => {
    ...
    expect(config.tiers.flash).toBe("local-qwen");
    expect(config.models["local-qwen"].protocol).toBe("openai");
    expect(config.models["local-qwen"].temperature).toBe(0.2); // default applied
  });
  ...
  it("creates default config on first run when global doesn't exist", () => {
    ...
    expect(config.tiers.flash).toBe("default-local");
  });
  ...
  it("merges local workspace config over global", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ tiers: { flash: "claude-sonnet" } })
    );
    const config = loadConfig({ globalConfigPath: GLOBAL_PATH, workspaceRoot: WORKSPACE });
    expect(config.tiers.flash).toBe("claude-sonnet"); // overridden
    expect(config.tiers.utility).toBe("claude-sonnet"); // preserved from global
  });
  ...
  it("throws when tier references nonexistent model", () => {
    const bad = { ...VALID_CONFIG, tiers: { flash: "nonexistent", utility: "claude-sonnet", dense: "claude-opus" } };
  ...
  it("throws on invalid protocol enum", () => {
    const bad = { tiers: { flash: "x", utility: "x", dense: "x" }, models: { x: { protocol: "invalid", model: "m", baseUrl: "http://x", contextLimit: 1000 } } };
    writeFileSync(GLOBAL_PATH, JSON.stringify(bad));
