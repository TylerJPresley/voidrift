...
- **REQ-G-3:** The gather phase SHALL always be interactive with model output streaming to the terminal in real time. The input prompt SHALL support line editing (arrow keys, backspace, history). IF a model request fails mid-session, THE SYSTEM SHALL print the error and return to the prompt (not exit). The interactive gather agent SHALL only have access to file-writing tools (`write_file`, `read_source_file`), not the full MCP tool set. Operator input SHALL be visually distinct from model output (colored prompt, blank line separation). The interactive agent SHALL use a conservative `max_tokens` limit (4096) to keep responses concise while allowing room for file writes.
  - *Rationale:* Exposing all 16 MCP tools in interactive mode causes local models to enter infinite tool-call loops instead of conversing. Limiting tools to write-only keeps the agent focused on dialogue. High token limits cause local models to generate for minutes per turn.
