...
- **REQ-RES-1:** Each phase prompt file (`resources/prompts/<phase>.md`) SHALL contain stage-specific instructions as H2 sections. The shared methodology for a phase SHALL be a skill file (e.g. ANALYSIS-REQS for gather) loaded once and prepended to every stage prompt. There are no separate role/agent files. The `resources/agents/` directory is deprecated.
