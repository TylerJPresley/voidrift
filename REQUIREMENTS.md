...
- **REQ-G-8:** WHEN `--from <path>` is specified, THE SYSTEM SHALL reverse-engineer requirements in four stages, each using a separate agent instance (per REQ-ARCH-7):
  1. **Triage:** Agent receives the full file tree (filtered only for runtime noise: `.git`, `node_modules`, `__pycache__`, `.venv`, `.cache`) and returns a JSON object with `groups` — a dict mapping logical boundary names to lists of source files worth analyzing (e.g. `{"backend": ["backend/main.py", ...], "frontend": ["frontend/src/App.vue", ...]}`). The triage agent — not the file tree filter — SHALL decide which files are source code vs. build artifacts, images, lock files, or generated output. The agent SHALL auto-detect logical boundaries from directory structure (e.g. `frontend/`, `backend/`, `api/`, `shared/`). Single-application codebases SHALL use one group.
