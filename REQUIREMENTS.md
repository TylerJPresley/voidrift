...
- **REQ-RES-1:** Role-specific agent files SHALL be loaded per phase via `get_agent()` MCP tool calls (per REQ-ARCH-6): Analyst for gather/chat, Architect for plan and escalations, Developer for develop/automate/verify. Agent files define the role identity, philosophy, and behavioral constraints. They SHALL NOT contain phase-specific pipeline instructions.
...
- **REQ-RES-7:** The CLI SHALL construct each agent's system prompt by concatenating: (1) the role from `get_agent()`, (2) the stage prompt from `get_prompt()`, and (3) any injected context (analyses, specs). The role is loaded once per pipeline and reused across stages.
...
- **REQ-G-8:** THE SYSTEM SHALL reverse-engineer requirements in four stages, each using a separate agent instance (per REQ-ARCH-7). Each agent's system prompt SHALL be constructed per REQ-RES-7: the Analyst role (loaded once via `get_agent("analyst")`) concatenated with the stage-specific prompt (loaded via `get_prompt("gather", "<stage>")`). Stage-specific instructions SHALL live in `resources/prompts/gather.md`, not hardcoded in the CLI.
