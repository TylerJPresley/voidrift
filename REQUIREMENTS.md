...
- **REQ-G-1:** `voidrift gather <model> <path>` SHALL reverse-engineer requirements from the codebase at `<path>` using the three-stage pipeline (REQ-G-8). `--overwrite` removes files from the previous gather run (per STATE.md manifest) before starting.
...
- **REQ-G-8:** THE SYSTEM SHALL reverse-engineer requirements in four stages, each using a separate agent instance (per REQ-ARCH-7). System prompts are stage-specific — only the consolidation stage loads the full ANALYSIS-REQS skill; per-file analysis and synthesis agents receive only the category-specific lens to minimise context overhead on local models (REQ-CFG-6). All instructions SHALL live in `resources/prompts/gather.md`.
  - *Rationale:* Three stages decompose a problem too large for one context window: triage categorizes and reduces the file set, per-file analysis keeps each context small, and synthesis produces requirements from the analyses. Separate agents per stage prevent context accumulation (REQ-ARCH-7). Predefined categories ensure the analysis lens matches the file's role in the project. Externalizing prompts to resource files allows methodology iteration without code changes. Per-file agents do not need the full ANALYSIS-REQS methodology — the category lens already captures the relevant guidance. Restricting analysis/synthesis prompts to the lens keeps local model context usage proportional to the task.
  1. **Triage:** Agent receives the complete file tree and returns a JSON object with files sorted into predefined categories — `source` (application code), `tests` (test files), `config` (build/project configuration), `infrastructure` (deployment, CI/CD, IaC), `documentation` (READMEs, ADRs, guides), `assets` (static resources, migrations, seeds). All categories are flat file lists. The file tree SHALL respect `.gitignore`: IF a `.gitignore` file exists in the target directory, the CLI SHALL exclude all paths matched by its patterns. Dot-prefixed files and directories (e.g. `.git/`, `.env`, `.voidrift/`) SHALL always be excluded regardless of `.gitignore`. The agent infers category membership from its knowledge of the project's language and toolchain; the CLI SHALL NOT hardcode language-specific rules. A second validation pass SHALL review the triage output and remove any files that were incorrectly included (e.g. build output, lock files, binary assets). The triage agent system prompt SHALL use the full ANALYSIS-REQS skill concatenated with the triage stage prompt.
  2. **Analysis:** One agent per file — reads the file via `read_source_file()`, stores a concise summary via `store_file_analysis()`, then exits. Each agent's system prompt SHALL contain only the category-specific lens and the analysis stage prompt — the full ANALYSIS-REQS skill SHALL NOT be included. The analysis lens SHALL vary by category: `source` — functional requirements, APIs, data flows, business logic; `tests` — behavioral expectations, acceptance criteria; `config` — constraints, toolchain requirements; `infrastructure` — deployment topology, operational requirements; `documentation` — documented design intent, decisions; `assets` — data requirements, schema. Each agent has a clean context containing only the single file. File content SHALL be truncated before being passed to the agent per REQ-G-13. Analysis output SHALL be concise per REQ-G-14. WHEN the API returns a 400 error indicating truncated tool call JSON, THE SYSTEM SHALL retry per REQ-G-15.
  3. **Synthesis:** One agent per file analysis extracts structured requirements. Each synthesis agent's system prompt SHALL contain only the category-specific lens and the synthesis stage prompt — the full ANALYSIS-REQS skill SHALL NOT be included. The CLI feeds analyses one at a time, ordered by category. Each agent calls `store_requirements()` to persist its extracted requirements. A fresh agent is used per analysis to prevent context accumulation (per REQ-ARCH-7).
  4. **Consolidation:** A single agent consolidates all extracted requirements into the final REQUIREMENTS.md. The consolidation agent system prompt SHALL include the full ANALYSIS-REQS skill (loaded via `get_skill("ANALYSIS-REQS")`) concatenated with the consolidation stage prompt. The agent calls `get_template("REQUIREMENTS-TEMPLATE")` then `write_framework_file("REQUIREMENTS.md")`. For multi-component projects it also writes `spec/<component>.md` files. The requirements SHALL be derived exclusively from the provided analyses.
  ...
  - Given a project with backend and frontend source files, When consolidation runs, Then `REQUIREMENTS.md` (overview) and `spec/backend.md` + `spec/frontend.md` (detailed requirements) are produced.
  - Given a single-component CLI tool, When consolidation runs, Then only `REQUIREMENTS.md` is produced with full detail.
  - Given test files with assertions about API behavior, When synthesis runs, Then those assertions appear as acceptance criteria in the relevant spec.
  - Given an analysis agent for a `source` file, When its system prompt is examined, Then it contains the source lens but NOT the full ANALYSIS-REQS skill text.
  - Given a consolidation agent, When its system prompt is examined, Then it contains the full ANALYSIS-REQS skill text.
  - Given a project with `src/main.py`, `Dockerfile`, `README.md`, and `tests/test_api.py`, When triage runs, Then each file is placed in the correct category (`source`, `infrastructure`, `documentation`, `tests`).
  - Given a project with backend and frontend source files, When synthesis runs, Then `REQUIREMENTS.md` (overview) and `spec/backend.md` + `spec/frontend.md` (detailed requirements) are produced.
  - Given a single-component CLI tool, When synthesis runs, Then only `REQUIREMENTS.md` is produced with full detail.
  - Given test files with assertions about API behavior, When synthesis runs, Then those assertions appear as acceptance criteria in the relevant spec.
...
- **REQ-G-12:** WHEN a gather stage requires tool calls (synthesis), THE SYSTEM SHALL use non-streaming mode (`stream=False`) for the agent instance. Stages that do not require tool calls (triage, analysis text output) MAY use streaming.
  - *Rationale:* vLLM's streaming tool call parser does not reliably separate text content from tool calls when the model outputs both in the same response. Non-streaming mode allows vLLM to parse the complete response at once, ensuring tool calls like `write_framework_file()` are correctly extracted and executed. The analysis stage already uses non-streaming successfully.
  - Given a synthesis agent calls `write_framework_file()` to create a spec, When the agent runs with `stream=False`, Then the tool call is parsed correctly and the file is written to disk.
  - Given a synthesis agent calls `write_framework_file()` to create REQUIREMENTS.md, When the agent runs with `stream=False`, Then the tool call is parsed correctly and the file is written to disk.
- **REQ-G-13:** BEFORE passing file content to an analysis agent, THE SYSTEM SHALL truncate it to the configured character limit (REQ-CFG-6 `limits.max_input_chars`). The default SHALL be 8000 characters for local models and unlimited for cloud models. Truncation SHALL append a `\n[truncated]` marker so the agent knows the file was cut. IF a file is truncated, the fact SHALL be logged.
  - *Rationale:* Large files cause analysis agents to generate proportionally large responses. For local models with limited output budgets this produces truncated tool call JSON (HTTP 400) rather than a graceful partial result. Truncating input keeps output proportionate to the token budget.
  - Given a local model is configured and a source file is 20,000 characters, When the analysis agent receives the file content, Then the content is truncated to the configured limit with a `[truncated]` marker.
  - Given a cloud model is configured, When the analysis agent receives a 20,000 character file, Then no truncation occurs.
...
| V-G-3 | REQ-G-12 | Inspection | `gather.py` — synthesis agent uses `stream=False` |
...
| V-G-4 | REQ-G-13 | Test | `test_phases.py` — local model file content truncated at configured limit; cloud model not truncated |
