...
- **REQ-MCP-3a:** ALL phase executions SHALL be scoped to a run ID. The run ID SHALL be the log filename stem (e.g. `gather-20260318-101048`). The CLI SHALL pass the run ID to the MCP artifact store via `artifacts.run_id` immediately after `_boot()`. ALL ephemeral MCP data (analyses, session logs, intermediate artifacts) SHALL be stored under `.voidrift/runs/<run_id>/`. `get_all_analyses()` and other retrieval tools SHALL return only data for the current run. WHEN any phase starts, THE SYSTEM SHALL auto-prune run directories older than the most recent 5. `voidrift log --prune` SHALL also delete run directories matching pruned log files.
