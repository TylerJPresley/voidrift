...
- **REQ-ARCH-14:** The agent loop SHALL support optional hook callbacks and loop-control mechanisms. All hooks default to None (inactive). WHEN a hook is provided, the loop SHALL call it at the specified evaluation point. Hooks:
  ...
  - Given `before_tool_call` returns `"blocked"` for `write_source_file`, When the tool would execute, Then `"blocked"` is used as the result and the tool handler is not called.
...
- **REQ-ARCH-15:** The agent loop SHALL provide a `snip_old_tool_results(messages, max_age_turns=2)` function that replaces old read-tool results with placeholders to free context. A tool result is eligible for snipping WHEN: it is from `read_source_file` or `read_framework_file`, at least `max_age_turns` assistant responses have appeared after it, and its content exceeds 500 characters. The function SHALL return a new list — the original messages SHALL NOT be modified. `max_age_turns=0` disables snipping. The function is designed for use as a `transform_context` hook in automated commands (develop, plan).
  ...
  - Given a read_framework_file result from 3 turns ago with 2000 characters, When snip runs with max_age_turns=2, Then the result is replaced with a placeholder.
  - Given a write_source_file result from 5 turns ago, When snip runs, Then the result is NOT snipped (write tools are never snipped).
  - Given a read_source_file result from 1 turn ago, When snip runs with max_age_turns=2, Then the result is NOT snipped (too recent).
- **REQ-ARCH-16:** WHEN multiple tool calls arrive in a single assistant message, THE SYSTEM SHALL partition them into batches for execution. Consecutive calls to concurrent-safe tools (`read_source_file`, `read_framework_file`, `list_project_artifacts`, `web_fetch`) SHALL form a single concurrent batch executed via `ThreadPoolExecutor`. Write tools and other non-safe tools SHALL each form their own serial batch. Results SHALL be returned in the original tool call order regardless of completion order. WHEN a `before_tool_call` hook is set, all batches SHALL execute serially to preserve hook semantics.
  - *Rationale:* When a develop agent reads the spec and the module arch in one turn, sequential execution adds unnecessary latency. Concurrent reads complete in the time of the slowest read, not the sum.
...
- **REQ-ARCH-17:** The agent loop SHALL normalize tool arguments after JSON parsing and before handler execution. Normalizers SHALL strip leading `/` and `./` from file path arguments and convert list-typed `content` arguments to newline-joined strings. WHEN normalization changes an argument, THE SYSTEM SHALL log `[TOOL_NORMALIZE tool=NAME field=FIELD raw=X normalized=Y]`. Normalizers SHALL NOT raise exceptions — they return best-effort normalized dicts.
  ...
  - Given `read_source_file` called with `path="/src/api.py"`, When normalization runs, Then path becomes `"src/api.py"` and the change is logged.
  - Given `write_source_file` called with `content=["line1", "line2"]`, When normalization runs, Then content becomes `"line1\nline2"`.
...
- **REQ-ARCH-9:** `build_local_tools()` SHALL accept an optional `cmd` parameter. WHEN a command name is specified, THE SYSTEM SHALL return only the agent tools relevant to that command. WHEN cmd is empty or omitted, all agent tools SHALL be returned. Each framework command SHALL pass its name as the `cmd` parameter: `gather`, `plan`, `develop`, `chat`. Agent tools SHALL enforce domain separation by name: `write_source_file` and `read_source_file` operate on the project source tree, `write_framework_file` and `read_framework_file` operate on `.voidrift/` artifacts. Per-command agent tool sets:
  ...
  - **Develop (per-task agent):** `read_source_file`, `write_source_file`, `edit_source_file`, `read_framework_file`, `run_command`. No skill agent tools — pre-injected per-task. No task orchestration agent tools — called directly by framework Python code.
  - **Chat:** `read_source_file`, `write_source_file`, `edit_source_file`, `read_framework_file`, `write_framework_file`, `list_project_artifacts`, `get_skill`, `list_skills`, `web_fetch`, `run_command`.
...
- **REQ-D-15:** WHEN a develop task is dispatched, THE SYSTEM SHALL create a per-task snapshot dict tracking original file state. Before the first write to any file path via `write_source_file` or `edit_source_file`, the original content SHALL be recorded (or `None` if the file does not exist). WHEN a task fails (exception or no-write exhaustion), all files in the snapshot SHALL be rolled back: files with original content are restored, files that did not exist are deleted. Each rollback action SHALL be logged as `[ROLLBACK]`. WHEN a task succeeds, snapshots SHALL be cleared and written files persist. Each concurrent task SHALL have an isolated snapshot dict via thread-local storage.
...
- **REQ-VF-11:** THE SYSTEM SHALL provide `read_process_output(handle_id)` returning up to 500 buffered lines (newest kept on overflow) and `run_command(cmd, cwd)` returning stdout, stderr, and exit_code synchronously. The verify command's `run_command` SHALL use the same `create_run_command` factory from `tools/bash.py` (REQ-SEC-4) with `BashConfig(enabled=True)` and the verify-specific timeout from `config.yml`.
...
- **REQ-U-12:** THE SYSTEM SHALL provide an `ask_user_question(question, options)` agent tool that displays a question to the operator and returns their response. WHEN `options` is provided, THE SYSTEM SHALL display them as a numbered list. WHEN stdin is not a TTY, THE SYSTEM SHALL return `"[No operator present. Use your best judgment and document your decision in comments.]"` without blocking. The tool SHALL be available in the `chat` command only. WHEN the tool is called during a chat session, THE SYSTEM SHALL restore terminal input mode for the prompt and re-disable after the operator responds (same pattern as `web_fetch` confirmation).
  ...
  - Given a chat session, When the model calls `ask_user_question("Which database?", '["PostgreSQL", "SQLite"]')`, Then the operator sees the question with numbered options and their response is returned.
  - Given stdin is not a TTY, When `ask_user_question` is called, Then the fallback message is returned without blocking.
  - Given `cmd="develop"`, When `build_local_tools()` is called, Then `ask_user_question` is not in the returned tool set.
...
- **REQ-U-18:** THE SYSTEM SHALL provide a `search_history(query, limit)` agent tool that searches the raw `chat-session.jsonl` file by case-insensitive substring match on message content. The tool SHALL load all JSONL entries (including those before compaction boundaries) and return up to `limit` (default 5, maximum 10) matching entries sorted by recency (newest first). Each returned entry SHALL include timestamp, role, and content truncated to 2000 characters. WHEN no session file exists or no matches are found, THE SYSTEM SHALL return a descriptive message. The `search_history` tool SHALL be available in the `chat` command only.
  ...
  - Given a chat session with 20 messages and a query matching 3 of them, When `search_history("auth endpoint")` is called, Then up to 3 matching entries are returned with timestamps and roles.
  - Given a query that matches no entries, When `search_history("nonexistent")` is called, Then a "no matches found" message is returned.
  - Given no session file exists, When `search_history("anything")` is called, Then a "no session history" message is returned.
  ...
  - Given `limit=3` and 7 matching entries, When `search_history("query", limit=3)` is called, Then only the 3 most recent matches are returned.
  - Given `cmd="develop"`, When `build_local_tools()` is called, Then `search_history` is not in the returned tool set.

- **REQ-U-19:** THE SYSTEM SHALL provide a `read_document(path)` agent tool that extracts text from binary document formats and returns plaintext or markdown. Format detection SHALL use file extension: `.pdf` (via `pymupdf`), `.docx` (via `python-docx`), `.xlsx` (via `openpyxl`). All three libraries are soft dependencies — WHEN a required library is not installed, the tool SHALL return an error naming the missing package and the pip install command. WHEN the file extension is unsupported, the tool SHALL return an error listing supported formats. Path validation SHALL use the same project-root sandboxing as `read_source_file` (REQ-SEC-1). XLSX output SHALL be formatted as markdown tables (one per sheet). DOCX output SHALL preserve heading hierarchy using markdown heading syntax. The `read_document` tool SHALL be available in the `chat` and `gather` commands.
  ...
  - Given a `.pdf` file and `pymupdf` is installed, When `read_document("spec.pdf")` is called, Then extracted text is returned as plaintext.
  - Given a `.docx` file and `python-docx` is installed, When `read_document("reqs.docx")` is called, Then paragraphs and tables are returned with heading hierarchy preserved.
  - Given a `.xlsx` file and `openpyxl` is installed, When `read_document("data.xlsx")` is called, Then each sheet is returned as a markdown table.
  - Given a `.pdf` file and `pymupdf` is NOT installed, When `read_document("spec.pdf")` is called, Then an error is returned containing `pip install pymupdf`.
  - Given a `.txt` file, When `read_document("notes.txt")` is called, Then an error is returned listing supported formats.
  - Given `cmd="develop"`, When `build_local_tools()` is called, Then `read_document` is not in the returned tool set.
  - Given `cmd="chat"`, When `build_local_tools()` is called, Then `read_document` is in the returned tool set.
  - Given `cmd="gather"`, When `build_local_tools()` is called, Then `read_document` is in the returned tool set.

- **REQ-U-20:** THE SYSTEM SHALL provide a `code_analysis(path)` agent tool that parses a source file via tree-sitter and returns a compact JSON summary containing: `language`, `lines` (total line count), `imports` (list of import strings), `symbols` (list of exported functions, classes, and constants with name and line number), and `complexity` (count of branch points: `if`/`for`/`while`/`try`/`except`/`match`). Language detection SHALL use file extension mapped to tree-sitter grammar names. `tree-sitter` and per-language grammar packages are soft dependencies — WHEN tree-sitter or the required grammar is not installed, the tool SHALL return an error naming the missing package and the pip install command. Path validation SHALL use the same project-root sandboxing as `read_source_file` (REQ-SEC-1). The `code_analysis` tool SHALL be available in the `chat` and `gather` commands.
  ...
  - Given a Python file and `tree-sitter-python` is installed, When `code_analysis("src/api.py")` is called, Then JSON is returned with language, lines, imports, symbols, and complexity.
  - Given a `.js` file and `tree-sitter-javascript` is installed, When `code_analysis("app.js")` is called, Then JSON is returned with JavaScript-specific imports and symbols.
  - Given a Python file and `tree-sitter` is NOT installed, When `code_analysis("src/api.py")` is called, Then an error is returned containing `pip install tree-sitter`.
  - Given a Python file and `tree-sitter` is installed but `tree-sitter-python` is NOT, When `code_analysis("src/api.py")` is called, Then an error is returned containing `pip install tree-sitter-python`.
  - Given a file with an unmapped extension, When `code_analysis("data.xyz")` is called, Then an error is returned listing supported languages.
  - Given `cmd="develop"`, When `build_local_tools()` is called, Then `code_analysis` is not in the returned tool set.
  - Given `cmd="chat"`, When `build_local_tools()` is called, Then `code_analysis` is in the returned tool set.
...
- **REQ-UI-12:** `voidrift chat` SHALL accept a `--style` option with values `verbose` (default), `terse`, and `raw`. In `verbose` mode, each tool call name SHALL be displayed as a dim line during execution. In `terse` mode, tool calls SHALL be hidden; a summary count line SHALL appear after each tool round (e.g. `[3 tool calls: 2× read_source_file, 1× write_source_file]`). In `raw` mode, all Rich formatting SHALL be disabled and output written as plain text. The style affects display only — the system prompt is identical across all styles.
  - *Rationale:* Operators monitoring long sessions want minimal noise (terse). Operators debugging want to see every tool call (verbose). Operators piping output want no ANSI codes (raw).
...
- **REQ-SKL-9:** Skill files SHALL support an optional `allowed_tools` field in YAML frontmatter listing the tool names the skill is permitted to use. WHEN a skill with `allowed_tools` is active, THE SYSTEM SHALL block tool calls not in the list via the `before_tool_call` hook, returning an error naming the skill and listing allowed tools. WHEN `allowed_tools` is absent or null, no restriction is applied. WHEN `allowed_tools` is an empty list, all tool calls are blocked. In chat, the active skill is set when `get_skill` is called and cleared at the start of the next user turn. In develop, allowed tools are parsed from all task skills and enforced for the task's agent. Skill restrictions are additive on top of the command's tool set — they can only reduce, never expand.
  ...
  - Given a skill with `allowed_tools: [read_source_file]` is active, When `write_source_file` is called, Then the call is blocked with an error listing allowed tools.
...
- **REQ-FSZ-4:** THE SYSTEM SHALL provide an `edit_source_file(path, old_str, new_str)` agent tool for surgical file modifications. The tool SHALL read the file at `path`, find the exact match of `old_str`, replace it with `new_str`, and write the result back. IF `old_str` appears more than once in the file, THE SYSTEM SHALL reject the edit and return an error containing the occurrence count and an instruction to add surrounding lines for uniqueness. IF `old_str` is not found, THE SYSTEM SHALL attempt a whitespace-normalized match (strip leading/trailing whitespace per line) and apply the replacement preserving the file's original indentation at the match site. IF no match is found after normalization, THE SYSTEM SHALL return an error containing the first 3 lines of `old_str` for debugging. Each successful edit SHALL increment the source write counter (counted as a write for REQ-D-5). The `edit_source_file` tool SHALL be available in the `develop` and `chat` command tool sets. The tool's `_guidelines` SHALL include: "Always call `read_source_file` before `edit_source_file` to confirm the exact string. Never guess at old_str content." The develop prompt SHALL instruct models to use `edit_source_file` for modifications to existing files and `write_source_file` for new files or full rewrites.
  - *Rationale:* `write_source_file` sends entire file content through the API even for a one-line change — ~2000 output tokens for a 10-token edit. Over a 50-task develop session with mostly modifications, this wastes 100K+ output tokens. Surgical edits reduce output token cost by 10-50x for modification tasks. Whitespace-normalized fallback handles models that reformat indentation in `old_str`. Ambiguity rejection prevents silent wrong-location edits when `old_str` matches multiple sites. The natural guard against external modification is that `old_str` won't match if the file changed.
  - Given a file `src/main.py` containing `x = 1`, When `edit_source_file("src/main.py", "x = 1", "x = 2")` is called, Then the file contains `x = 2` and the write counter increments.
  ...
  - Given `cmd="develop"`, When `build_local_tools()` is called, Then `edit_source_file` is in the returned tool set.
  - Given `cmd="plan"`, When `build_local_tools()` is called, Then `edit_source_file` is not in the returned tool set.

- **REQ-FSZ-5:** WHEN `read_source_file` or `read_framework_file` returns content, THE SYSTEM SHALL apply a secondary byte-count guard after line pagination (REQ-FSZ-1). WHEN the UTF-8 encoded result exceeds `max_read_bytes` (default 524288 — 512KB), THE SYSTEM SHALL truncate at a valid UTF-8 boundary and append a marker containing the total size, the shown size, and pagination instructions. Setting `max_read_bytes` to 0 disables the byte guard. The byte guard applies after line pagination — both limits are independent.
  ...
  - Given a file whose paginated result is exactly 512KB, When `read_source_file` is called, Then no byte truncation occurs.
  - Given a file whose paginated result is 600KB, When `read_source_file` is called, Then the result is truncated at 512KB with a marker.
...
- **REQ-SEC-1:** All file tool operations SHALL validate that resolved paths remain within their designated root directory using `Path.resolve().relative_to()`. Source tools validate against the project directory. Framework tools validate against `.voidrift/`. Path violations SHALL be logged as `[PATH_BLOCKED path=X reason=outside_root]`. Write tools (`write_source_file`, `edit_source_file`) SHALL additionally check a `protected_paths` set — paths in this set are blocked from writes even within the project root, logged as `[PATH_BLOCKED path=X reason=protected]`. `config.yml` SHALL support a `protected_paths` list. `configure()` SHALL accept `protected_paths` and `log_path` parameters.
  ...
  - Given `write_source_file("../../etc/hosts", ...)`, When the path is validated, Then the write is blocked and logged.
  - Given `pyproject.toml` in `protected_paths`, When `write_source_file("pyproject.toml", ...)` is called, Then the write is blocked.
...
- **REQ-SEC-2:** THE SYSTEM SHALL classify shell commands before execution via `classify_command(command, allowed_commands)` returning a risk level (`safe`, `warn`, `block`) and reasons. Block patterns: destructive deletes (`rm -rf /`), fork bombs, disk overwrites (`dd if=/dev/zero`), filesystem formats (`mkfs`), remote code execution (`curl|bash`), writes to system paths (`/etc/`, `/boot/`, `/sys/`, `/proc/`). Warn patterns: recursive force delete, force push, hard reset, sudo, recursive chmod. `allowed_commands` in `config.yml` provides glob patterns that override classification to safe. Blocked commands SHALL NOT execute and SHALL return an error. All command executions SHALL be logged as `[CMD_EXEC cmd=X classification=LEVEL]`.
  - *Rationale:* `run_command` accepts arbitrary shell commands. Without classification, models can produce destructive commands. This is defense-in-depth, not a comprehensive security boundary.
...
- **REQ-SEC-4:** THE SYSTEM SHALL provide a `bash` configuration section in `config.yml` with per-command subsections (`develop`, `chat`, `verify`). Each subsection SHALL support `enabled` (boolean, default `true` for verify, `true` for develop and chat), `allowed_patterns` (list of glob patterns restricting which commands the agent can run), and `timeout` (integer seconds, default 120). WHEN `bash.<command>.enabled` is `false`, THE SYSTEM SHALL reject all `run_command` calls for that command with an error. WHEN `bash.<command>.allowed_patterns` is non-empty, THE SYSTEM SHALL reject commands not matching any pattern before applying the global `classify_command` check (REQ-SEC-2). The global `allowed_commands` and `classify_command` remain the second security layer — per-command patterns can only narrow, never widen, what is permitted. `create_run_command(config, global_allowed, log_path)` in `tools/bash.py` SHALL be a factory that returns a `run_command(cmd, cwd)` handler bound to a `BashConfig`. Output exceeding `max_output_lines` (default 500) SHALL be truncated with a count of omitted lines.
  ...
  - Given `bash.develop.enabled: true` and `allowed_patterns: ["pytest *"]`, When a develop agent calls `run_command("pytest tests/")`, Then the command executes and returns stdout/stderr/exit_code.
  - Given `bash.develop.allowed_patterns: ["pytest *"]`, When a develop agent calls `run_command("git push --force")`, Then the command is rejected with an allowed_patterns error before security classification runs.
  - Given `bash.chat.enabled: false`, When a chat agent calls `run_command("make test")`, Then the command is rejected with a disabled error.
  - Given `bash.develop.enabled: true` and no `allowed_patterns`, When a develop agent calls `run_command("rm -rf /")`, Then the command is blocked by `classify_command` (REQ-SEC-2).
  - Given output of 1000 lines and `max_output_lines: 500`, When `run_command` returns, Then stdout is truncated to 500 lines with a truncation message.
...
- **REQ-MEM-1:** THE SYSTEM SHALL provide a two-layer memory system for persisting knowledge across chat sessions. Project memory SHALL live in `<project>/.voidrift/memory/` and contain project-specific facts, conventions, and decisions. Global memory SHALL live in `~/.voidrift/memory/` and contain operator preferences shared across all projects. Each memory entry SHALL be a markdown file with YAML frontmatter (`name`, `scope`, `description`, `created`, `updated`). A `MEMORY.md` index file in each layer SHALL list all entries with name and description. WHEN chat starts (non-bare), THE SYSTEM SHALL inject the combined index (global then project) into the system prompt as a `## Project Memory` block containing names and descriptions only — not full content. Project entries with the same name as global entries SHALL override the global entry in the index. The chat command SHALL provide `read_memory(name)`, `write_memory(name, content, scope)`, and `list_memory()` agent tools. `read_memory` SHALL search project first, then global. `write_memory` scope defaults to `project`. Memory tools SHALL be available in the chat command only. `--bare` mode SHALL skip memory injection. THE SYSTEM SHALL provide a `voidrift memory` subcommand group with: `list` (display all entries grouped by layer), `show <name>` (print full entry content, searching project then global), and `delete <name>` (remove an entry, `--global` to target global layer). These commands SHALL NOT require a model — they operate on local files only.
  ...
  - Given `read_memory("code-style")` and the entry exists only in global, When called, Then the global entry content is returned.
  - Given `write_memory("api-notes", "...", scope="project")`, When called, Then the entry is written to `.voidrift/memory/` and the project MEMORY.md index is updated.
...
| V-ARCH-13 | REQ-ARCH-13 | Test | `test_agent.py::TestTokenBudget` — budget records usage, raises BudgetExhaustedError when exceeded, no-op when unconfigured; CLI flags override config |
...
| V-ARCH-16 | REQ-ARCH-16 | Test | `test_agent.py::TestConcurrentTools` — consecutive reads batched; writes serial; results in original order |
| V-ARCH-17 | REQ-ARCH-17 | Test | `test_agent.py::TestToolNormalize` — leading / stripped; content list joined; changes logged |
| V-ARCH-18 | REQ-ARCH-18 | Inspection | `agent.py::_run_loop` — `[ITERATION]` at every continue; `[LOOP_EXIT]` at every return |
