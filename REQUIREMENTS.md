...
- **REQ-MCP-5:** ~~REMOVED~~ — Role files replaced by ROLE section in phase prompt files (REQ-RES-1).
...
- **REQ-RES-1:** Each phase prompt file (`resources/prompts/<phase>.md`) SHALL contain a `## ROLE` section defining the agent's identity, philosophy, and behavioral constraints for that phase. There are no separate role/agent files. The `resources/agents/` directory is deprecated.
...
- **REQ-RES-6:** Phase prompt files SHALL live in `resources/prompts/<phase>.md` with H2 sections for each stage/step (including `## ROLE`). The CLI SHALL load prompts via `get_prompt(phase, section)` MCP tool calls (per REQ-ARCH-6). Prompts MAY contain Python format variables (e.g. `{spec_path}`, `{group_name}`) which the CLI resolves via `.format()` before passing to the agent. A missing variable SHALL raise a `KeyError` — no silent substitution.
- **REQ-RES-7:** The CLI SHALL construct each agent's system prompt by concatenating: (1) the ROLE section from `get_prompt(phase, "ROLE")`, (2) the stage prompt from `get_prompt(phase, section)`, and (3) any injected context (analyses, specs). The role is loaded once per pipeline and reused across stages.
...
- **REQ-G-8:** THE SYSTEM SHALL reverse-engineer requirements in four stages, each using a separate agent instance (per REQ-ARCH-7). Each agent's system prompt SHALL be constructed per REQ-RES-7: the ROLE section (loaded once via `get_prompt("gather", "ROLE")`) concatenated with the stage-specific prompt (loaded via `get_prompt("gather", "<stage>")`). All instructions SHALL live in `resources/prompts/gather.md`, not hardcoded in the CLI.
