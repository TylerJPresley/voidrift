...
- **REQ-G-8:** WHEN `--from <path>` is specified, THE SYSTEM SHALL reverse-engineer requirements in four stages, each using a separate agent instance (per REQ-ARCH-7):
  1. **Triage:** Agent receives the file tree and returns a JSON object with two fields: `groups` (a dict mapping logical boundary names to lists of source files, e.g. `{"backend": ["backend/main.py", ...], "frontend": ["frontend/src/App.vue", ...]}`) and `files` (flat list of all selected files for backward compat). The agent SHALL auto-detect logical boundaries from directory structure (e.g. `frontend/`, `backend/`, `api/`, `shared/`). Single-application codebases SHALL use one group.
