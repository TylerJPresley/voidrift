...
**Auto mode:** Select "auto" to let the router adapt to the agent type. Interactive agents default to Flash, task agents default to Utility. Dense activates via escalation — either the model requests it (calls `escalate` tool) or context exceeds 85%.
...
**Model-requested escalation:** The model can call `escalate` when a task exceeds its reasoning capacity. This requires user approval. The model calls `deescalate` to return to Utility when done.
