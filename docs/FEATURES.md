...
VoidRift has three core operating modes plus custom agents. Switch with **Shift+Tab** (cycles all active interactive agents).
...
| `/config [global]` | Edit configuration with validation and hot-reload |
...
| `/skills` | Skill registry — browse, create, rename |
| `/skills create <name> [global]` | Create a new skill |
| `/skills rename <old> <new>` | Rename a skill |
| `/agents` | Agent manager — browse, create, rename |
| `/agents create <id> [task] [global]` | Create a new agent |
...
| `/templates` | Template registry — create, rename, browse |
| `/templates create <name> [global]` | Create a new template |
| `/templates rename <old> <new>` | Rename a template |
| `/prompts` | Prompt manager — view and override system prompts |
| `/run <instruction>` | Autonomous execution — ad-hoc one-off |
| `/run routine <name>` | Execute a saved routine |
| `/run plan <name>` | Execute a plan autonomously |
| `/routines` | Routine manager — repeatable instructions |
| `/schedule` | Background timer and cron task manager |
| `/tasks` | Background task monitor — active processes |
...
| `/reflection` | Analyze past sessions, extract lessons into memory |
...
**Why it matters:** Without a plan, conversations are ephemeral — the model forgets what you agreed to do. Plans create a persistent roadmap. When the model has an active plan in context, it works systematically instead of ad-hoc. Plans are also the input for `/run plan` autonomous execution.

**Walkthrough:**

1. Type `/plan` to open the plan panel
2. The panel shows four priority pages: **now** (immediate), **next** (upcoming), **later** (backlog), **complete** (finished plans with summary and actions taken)
...
**Model integration:** The model has `read_plan`, `add_plan`, `remove_plan`, `prioritize_plan`, and `update_plan` tools. It can create items, change priorities, and update bodies as it works. This means you can say "plan out the refactor" and it produces structured items you can review and reprioritize.
...
**Intent:** Analyze past sessions and extract reusable lessons into memory. This is how VoidRift learns from experience — turning patterns, errors, and decisions into persistent knowledge.

**Why it matters:** After working on a project for several sessions, patterns emerge: certain approaches work better, certain tools fail in specific ways, certain conventions matter. Reflection mines that history and crystallizes the important bits into memories that inform future sessions.

**Walkthrough:**

1. Type `/reflection` and press Enter
2. VoidRift reads all past sessions in `.voidrift/sessions/`
3. Batches session histories and sends them to the active model for analysis
4. Progress streams to the TUI: `Reflecting on N past sessions...`
5. Each extracted lesson shows as: `📝 Lesson title`
6. Lessons are saved as memory files in `.voidrift/memory/`
7. Duplicate lessons (keyword overlap with existing memories) update in place rather than creating duplicates

**Output:** A summary showing total sessions analyzed and lessons extracted. Lessons are immediately available as memories for future sessions.

**When to use:**
- After a productive multi-session work sprint
- When you notice the model keeps making the same mistake
- Periodically (weekly/biweekly) to accumulate project knowledge
...
- `agent.json` — Configuration (model tier, tools, approval mode, type, resources)
- `prompt.md` — Identity/persona instructions

**Resources:** Agents declare what they need via dedicated config fields:
- `skills` — Array of skill names to always load (must be registered in `.voidrift/skills/` or `~/.config/voidrift/skills/`)
- `templates` — Array of template keys bound to this agent (must be registered)
- `resources` — Array of `file://` URIs loaded into context on activation

```json
{
  "skills": ["react-patterns", "testing-conventions"],
  "templates": ["pr-description"],
  "resources": [
    "file://./src/api/README.md",
    "file:///home/user/.config/voidrift/context/global-rules.md"
  ]
}
```
...
**Intent:** Select which model handles your conversations. Your choice persists per-project.

**Why it matters:** Different models have different tradeoffs. A local 7B model is instant and free but less capable. Claude Opus is powerful but expensive and slow. You select the model that fits your current task, and the harness remembers your choice.

**Walkthrough:**

**Quick switch:** `/model claude-sonnet` — immediately switches to that model.

**Panel mode:** `/model` (no arguments) opens the full panel:

1. See all configured models with their current tier assignments
2. The currently selected model shows a `✓` prefix
...
**Intent:** Execute work autonomously using the Ralph Loop — the model works independently until completion.

**Usage:**
- `/run <instruction>` — ad-hoc one-off execution
- `/run routine <name>` — execute a saved routine from `.voidrift/routines/`
- `/run plan <name>` — execute a plan from `.voidrift/plan/`

**Walkthrough:**

1. Type `/run refactor src/auth to use JWT tokens`
2. VoidRift enters an autonomous loop:
   - Read relevant files
   - Plan approach
   - Make changes
...
3. Progress updates appear as `[Run]` messages
4. On completion: reports success/failure, turns used, and termination reason

**Termination conditions:**
- Model declares the work complete
- Token budget exhausted
- Error count exceeds threshold
- User cancels with `esc`

**Note:** In Chat mode, execution still respects the permission gate — you'll approve file writes and commands. In Vibe mode, it runs fully autonomous.

---

#### `/routines` — Repeatable Instructions

**Intent:** Manage standing instructions that you execute on demand. Routines are permanent recipes — they never get consumed or marked complete.

**Storage:** `.voidrift/routines/<name>.md`
...
## Done When
Staging URL returns 200 and smoke tests pass
```

**Panel actions:**
- `r` — Run (executes via Ralph Loop)
- `c` — Create new routine
- `del` — Delete
- `enter` — View/edit details

**Difference from plans:** Plans have lifecycle lanes (now/next/later/complete) and get consumed. Routines are permanent — you run them repeatedly without modification.
...
**Cron format:** Simplified interval patterns only — `*/N` in the minute or hour field (e.g., `*/5 * * * *` = every 5 minutes). Full 5-field cron with specific days/months is not supported.

**Viewing tasks:** Type `/tasks` to see all active background tasks, subagents, and scheduled timers.
...
**Why it matters:** When you have autonomous work running (run loops, scheduled tasks, subagents), you need visibility into what's happening. This panel shows what's running, what's pending, and what completed.
...
## Startup Validation & Callouts

VoidRift runs a validation pass on startup and displays issues as **callouts** — colored inline messages in the chat history.

### Callout Levels

| Level | Icon | Color | Meaning |
|-------|------|-------|---------|
| Warning | ⚠ | Yellow | Something is misconfigured but the app still works |
| Info | ℹ | Blue | Non-critical status information |
| Error | ✗ | Red | Something failed that may affect functionality |

### What Gets Validated

**Agent resources:**
- Skills declared in the `skills` array must exist in `.voidrift/skills/` or `~/.config/voidrift/skills/`
- Unknown skill names produce a warning: `⚠ [agent] my-agent: References unknown skill: "nonexistent"`

**Skill files:**
- Must have YAML frontmatter with `name`, `description`, `triggers`, `agents`, `active` fields
- Missing fields produce a warning: `⚠ [skill] my-skill: Missing required frontmatter keys: triggers, active`
- Skills referencing non-existent agents produce a warning

**Editor:**
- If `editor` is set in config, VoidRift checks the command exists on PATH
- Missing editor produces a warning (panels that open files won't work)

### Example Startup

```
⚠ [agent] backend-dev: References unknown skill: "api-standards"
⚠ [skill] old-skill: Missing required frontmatter keys: agents
⚠ Editor "subl" not found. Command "subl" is not in PATH.
```

These appear immediately after the welcome banner. Fix the referenced issues and restart — the warnings disappear.
...
| `workspace_map` | Explore directory structure and file symbols on demand | Auto |
...
| `add_plan` | Create a new plan item | Auto |
| `remove_plan` | Delete a plan item by name | Auto |
| `prioritize_plan` | Move a plan item to now/next/later | Auto |
| `update_plan` | Edit a plan item's body via search/replace | Auto |
...
| `delete_memory` | Delete a memory by ID | Auto |
| `list_memory` | Browse all saved memories — titles, summaries, keywords | Auto |
| `list_skills` | Browse all available skills — names, descriptions, triggers | Auto |
...
- `type` (optional) — `directive` (always in context) or `reference` (loaded on demand, default)

---

| Tool | Description | Approval |
|------|-------------|----------|
| `escalate` | Request escalation to the Dense model | Gated |
| `deescalate` | Return to the Utility model | Auto |
...
| `register_task` | Register a reusable task definition for batch work | Auto |
| `invoke_task` | Run a registered task with input variables | Auto |
...
│    Always allow (project), "src/main.ts"              │
│    Always allow (user), "src/main.ts"                 │
│    Allow this session, "src/**"                       │
...
- **Allow this session** — Approve this pattern for the rest of the session (lost on restart).
- **Always allow (project)** — Persist rule to `.voidrift/config.json`. Shared with team, scoped to this workspace.
- **Always allow (user)** — Persist rule to `~/.config/voidrift/config.json`. Personal, applies across all workspaces.
...
| Shell commands | `bun run test` | `bun *` |
| MCP tools | `mcp_github_search_repos` | `mcp_github_*` |

### Policy Rules

Rules persist in `config.json` under the `policies` array (workspace or global scope).
...
| `Shift+Tab` | Input | Cycle active interactive agents (Chat → Plan → Vibe → custom agents) |
| `Ctrl+V` | Input | Attach clipboard image |
| `Ctrl+X` | Input (image attached) | Remove attached image |
...
- **Clipboard paste**: `Ctrl+V` when clipboard contains an image (Linux: wl-paste/xclip, macOS: osascript)
- **File path**: Include an image path in your message (e.g., `what's in ./screenshot.png`)
- Supported formats: PNG, JPG, JPEG, GIF, WebP, BMP
- If the model doesn't support images, a friendly error is shown
