...
*   **Read [PATTERNS.md](./PATTERNS.md) before writing code.** It defines the Clean Architecture rules, repository pattern, CoreAPI namespaces, and testing practices that all contributions must follow.
...
2.  **Add Tests**: If you are fixing a bug or adding a new feature, write corresponding tests in the `tests/` directory. Use in-memory repository implementations for unit tests.
3.  **Follow Architecture Rules**: See the [PR Checklist in PATTERNS.md](./PATTERNS.md#pr-checklist). Domain classes must not import `fs`. New API methods go on CoreAPI namespaces. Panels source data from CoreAPI only.
4.  **Self-Review**: Review your own changes and ensure no temporary debugging code, console logs (other than configured logging systems), or unused imports are committed.
5.  **Describe the PR**: Fill out the Pull Request template completely, linking any relevant GitHub issues.
6.  **Clean Git History**: Avoid merge commits in your branch. Rebase on `main` if necessary.
