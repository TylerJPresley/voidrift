...
import { FileSystemMemoryRepository } from "../../src/session/memory-repository.js";
...
describe("MemoryRegistry", () => {
  it("indexes memory files from directory", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
  ...
  it("Stage 1: discovers memories by file extension", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
    reg.index([MEMORY_DIR]);
    const discovered = reg.rankRelevantMemories({ focusedFiles: ["src/db.ts"], userInput: "" });
  ...
  it("Stage 1: discovers memories by filename", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
    reg.index([MEMORY_DIR]);
    const discovered = reg.rankRelevantMemories({ focusedFiles: ["schema.prisma"], userInput: "" });
    expect(discovered).toHaveLength(1);
    expect(discovered[0].id).toBe("mem-db-pool-01");
  });
  ...
  it("Stage 1: discovers memories by keyword in user input", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
    reg.index([MEMORY_DIR]);
    const discovered = reg.rankRelevantMemories({ focusedFiles: [], userInput: "fix the prisma pool issue" });
    expect(discovered).toHaveLength(1);
    expect(discovered[0].id).toBe("mem-db-pool-01");
  });
  ...
  it("Stage 1: discovers multiple matching memories", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
    reg.index([MEMORY_DIR]);
    const discovered = reg.rankRelevantMemories({ focusedFiles: ["component.tsx"], userInput: "prisma connection" });
  ...
  it("Stage 1: returns empty when nothing matches", () => {
    const reg = new MemoryRegistry(new FileSystemMemoryRepository());
    reg.index([MEMORY_DIR]);
    const discovered = reg.rankRelevantMemories({ focusedFiles: ["main.go"], userInput: "write a go server" });
