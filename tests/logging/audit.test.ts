...
describe("AuditLogger", () => {
  ...
  it("auto-logs TURN_BEFORE events when attached to bus", async () => {
    const bus = new EventBus();
    const logger = makeLogger("bus-test");
    logger.attach(bus);

    bus.publish("TURN_BEFORE", { userMessage: "fix the bug", activeTools: [], activePlan: null, recentSummaries: [], recentTools: [] } as any);
