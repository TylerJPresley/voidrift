...
describe("Streaming Engine", () => {
  ...
  it("extracts tool calls from accumulated chunk with tool_calls", async () => {
    const client = makeMockClient([
  ...
  it("assembles tool calls from tool_call_chunks", async () => {
    const client = makeMockClient([
  ...
  it("handles mid-stream errors (network dropout)", async () => {
  ...
  it("mid-stream error flushes accumulated tool calls", async () => {
    const client = makeMidStreamErrorClient(
      [{ content: "", tool_calls: [{ id: "tc1", name: "read_file", args: { path: "x" } }] }],
