...
describe("Integration: Run Loop", () => {
  it("runs until model outputs completion token", async () => {
    ...
    const bus = new EventBus();
    const result = await ralphLoop("process files", client, bus, () => {});
...
describe("Integration: Plugin Registration", () => {
  it("registers sandbox mode with path guard", () => {
    ...
    pi.register.agent({ id: "test", name: "Test", description: "test agent", type: "interactive", role: "auto", prompt: "You are a test.", tools: [], approvalMode: "prompt", allowedTools: [] });
  ...
  it("registers and emits custom events", () => {
    ...
    api.events.register("TASK_COMPLETED");
    let received: any = null;
    api.events.subscribe("TASK_COMPLETED", (e) => { received = e; });
    api.events.emit("TASK_COMPLETED", { taskId: "t1" });
  ...
  it("core api exposes workspace map and command execution", () => {
    ...
    const result = api.workspace.exec("echo hello");
    expect(result.success).toBe(true);
