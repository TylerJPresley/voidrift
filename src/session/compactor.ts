 ...
 * Keeps last N turns at full fidelity, compresses older turns using
...
export function compactHistory(
  ...
  const keepCount = recentKeep ?? RECENT_TURNS_KEPT;
  const threshold = contextLimit * 0.75;

  if (usedTokens <= threshold || messages.length <= keepCount) {
    return { messages, compacted: false, preservedDiagnostics: [], preservedParameters: [] };
  }

  // Find a safe split point — never split between an assistant message with toolCalls and its tool results
  let splitIdx = messages.length - keepCount;
  // If the split lands on a tool message, back up to include its assistant parent
  while (splitIdx > 0 && messages[splitIdx]?.role === "tool") {
    splitIdx--;
  }
  // If the split lands right after an assistant with toolCalls, move forward past the tool results
  if (splitIdx > 0 && messages[splitIdx - 1]?.role === "assistant" && messages[splitIdx - 1]?.toolCalls?.length) {
