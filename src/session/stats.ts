...
export class StatsTracker {
  ...
  constructor(sessionId: string) {
    this.stats = {
      ...
      turns: 0,
      modelUsage: new Map(),
      tools: { total: 0, success: 0, failed: 0, totalTimeMs: 0, perTool: new Map() },
    };
  }
  ...
  recordToolCall(success: boolean, timeMs: number, toolName?: string): void {
    this.stats.tools.total++;
    if (success) this.stats.tools.success++;
    else this.stats.tools.failed++;
    this.stats.tools.totalTimeMs += timeMs;
    if (toolName) {
      this.stats.tools.perTool.set(toolName, (this.stats.tools.perTool.get(toolName) || 0) + 1);
    }
  }
