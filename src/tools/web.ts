 ...
 */
export async function webFetch(url: string, workspaceRoot?: string, opts?: { timeoutMs?: number; smallFileLines?: number }): Promise<WebResult> {
  try {
    ...
    if (lines.length <= (opts?.smallFileLines ?? SMALL_FILE_THRESHOLD)) {
...
export async function webSearch(query: string, config?: SearchConfig): Promise<WebResult> {
  ...
  if (provider === "google" && config?.apiKey) {
    return googleSearch(query, config.apiKey);
  }
...
async function googleSearch(query: string, apiKey: string): Promise<WebResult> {
  try {
    const encoded = encodeURIComponent(query);
    const url = `https://www.googleapis.com/customsearch/v1?q=${encoded}&key=${apiKey}&num=8`;
    ...
    if (!response.ok) {
      return { output: "", error: `Google API error: HTTP ${response.status}` };
    }
    ...
    if (err.name === "AbortError") return { output: "", error: "Search timed out" };
    return { output: "", error: err.message || "Google search failed" };
  }
}
