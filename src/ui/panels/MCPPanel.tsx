...
export function MCPPanel({ core, onClose }: { core: CoreAPI; onClose: () => void }) {
  const config = core.workspace.config();
  const mcp = { loadConfigs: () => core.mcp.listConfigs(), all: core.mcp.list().servers as any[], connected: core.mcp.list().servers.filter((s: any) => s.status === "connected"), connect: (cfg: any) => core.mcp.connect(cfg.name || cfg), disconnect: (n: string) => core.mcp.disconnect(n), removeConfig: (n: string) => core.mcp.removeConfig(n) };
  ...
  const getToolCount = (name: string) => servers.find(s => s.name === name)?.toolCount ?? 0;
  ...
  useInput((input, key) => {
    ...
    if (detail) {
      ...
      if (input === "e" && config.editor) {
        setMessage(`Edit via /config — MCP servers are in config.json under "mcp.${detail}"`);
      }
      if (input === "a") {
        // Toggle autoConnect
        const cfg = mcp.loadConfigs().find(c => c.name === detail);
        if (cfg) {
          const newVal = (cfg as any).autoConnect === false;
          core.mcp.saveConfig({ ...cfg, name: detail!, autoConnect: newVal } as any);
          setMessage(`autoConnect: ${newVal}`);
        }
      }
      if (input === "x") {
        ...
        if (st === "connected") {
          ...
          if (cfg) {
            ...
            mcp.connect(cfg).then(() => {
              const srv = core.mcp.list().servers.find((s: any) => s.name === detail);
              setMessage(srv?.status === "connected" ? `Connected (${srv.toolCount} tools)` : `Error: connection failed`);
            });
          }
        }
      }
  ...
  if (detail) {
    ...
    const location = mcp.loadConfigs().findIndex((c: any) => c.name === detail) >= 0 ? "config" : "unknown";
    return (
      <Box flexDirection="column" borderStyle="single" borderColor="#5a6aa8" paddingX={1}>
        ...
        <Text><Text color="#61afef">{"Location:".padEnd(16)}</Text>{location === "config" ? "workspace" : "unknown"}</Text>
        ...
        {tools.length > 0 && <>
          ...
          {tools.slice(0, 15).map((t: any) => <Text key={t.name}><Text color="#61afef">{"  " + t.name.padEnd(25)}</Text><Text dimColor>{t.description}</Text></Text>)}
          {tools.length > 15 && <Text dimColor>  ...{tools.length - 15} more</Text>}
        </>}
        {srv && srv.errorLog.length > 0 && <>
          <Text> </Text>
          <Text bold color="red">Errors</Text>
          {srv.errorLog.slice(-3).map((e: string, i: number) => <Text key={i} dimColor>  {e.trim().slice(0, 80)}</Text>)}
        </>}
        <Text> </Text>
  ...
  return (
    <Box flexDirection="column" borderStyle="single" borderColor="#5a6aa8" paddingX={1}>
      ...
      <Text dimColor>  Workspace:  .voidrift/config.json → mcp</Text>
      <Text dimColor>  Global:     ~/.config/voidrift/config.json → mcp</Text>
      <Text> </Text>
            ...
            import("../../mcp/discovery.js").then(({ discoverMCPServer, buildConfigFromDiscovery }) => {
              discoverMCPServer(v, setMessage).then((discovery) => {
                if (discovery.error) {
                  ...
                  core.mcp.saveConfig({ name, transport: "http-sse", url: v, autoConnect: true } as any);
                } else {
                  const cfg = buildConfigFromDiscovery(name, discovery);
                  core.mcp.saveConfig({ name, ...cfg, autoConnect: true } as any);
