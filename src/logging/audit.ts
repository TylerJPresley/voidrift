...
export class AuditLogger {
  ...
  attach(bus: EventBus): () => void {
    ...
    unsubs.push(bus.subscribe("TURN_BEFORE", (e) => {
      this.local("info", "user", "input", { text: (e.payload as any).userMessage });
    }));
...
/**
 * Query audit logs using Liqe (Lucene-like syntax).
 * Parses NDJSON log file, applies query filter, returns matching entries.
 */
export async function queryLogs(logPath: string, query: string, opts: QueryOptions = {}): Promise<QueryResult> {
  const { limit = 50, since } = opts;

  if (!existsSync(logPath)) return { entries: [], total: 0, query };
  ...
  if (query.trim()) {
    try {
      const liqe = await import("liqe") as { parse: (q: string) => unknown; filter: (ast: unknown, items: unknown[]) => unknown[] };
      const ast = liqe.parse(query);
      entries = liqe.filter(ast, entries) as LogEntry[];
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      return { entries: [], total: 0, query: `Error parsing query: ${msg}` };
    }
  }
