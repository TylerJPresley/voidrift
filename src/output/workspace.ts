...
/**
 * Gets the short git SHA of HEAD. Returns empty string if not in a git repo.
 */
export function getGitSha(cwd: string): string {
  try {
    return execSync("git rev-parse --short HEAD", { cwd, encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }).trim();
  } catch {
    return "";
  }
}
