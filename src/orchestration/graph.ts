...
export async function directChat(input: OrchestrationInput, bus?: EventBus): Promise<OrchestrationResult> {
  ...
  const gate = bus ? new PermissionGate(bus, _workspaceRoot, input.config?.approvalTimeout) : null;
  ...
  for (let round = 0; round < maxToolRounds; round++) {
    ...
    for (const tc of response.toolCalls) {
      ...
      if (tc.name === "read_file" && input.context && input.config) {
        ...
        const worktree = new WorktreeEngine(_workspaceRoot, bus!);
      ...
      currentMessages.push(new ToolMessage({ content: modelResult, tool_call_id: tc.id }));
      ...
      const contextLimit = input.config?.models[input.config.tiers[input.tier as keyof typeof input.config.tiers] ?? ""]?.contextLimit ?? 128000;
      const maxOutput = input.config?.models[input.config.tiers[input.tier as keyof typeof input.config.tiers] ?? ""]?.maxOutputTokens ?? 4096;
      if (currentTokens > (contextLimit - maxOutput) * 0.85) break;
    }
  }
