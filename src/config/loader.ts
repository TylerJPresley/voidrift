...
export const ConfigSchema = z.object({
  ...
  maxReadLines: z.number().positive().default(1000),
  maxConcurrentAgents: z.number().positive().default(1),
  approvalTimeout: z.number().min(0).default(120).describe("Seconds to wait for tool approval. 0 = no timeout."),
  decayAfterTurns: z.number().min(0).default(20).describe("Summarize messages older than N turns. 0 = no decay."),
