---
name: BACKEND-ENG
description: Python backend engineering conventions for the VoidRift framework — Click CLI, FastMCP tools, AgentLoop, pytest patterns.
---
...
- **Pull over push:** Agents retrieve context on demand via MCP tools rather than receiving pre-loaded blobs.
- **Phase discipline:** Each phase exposes only its required tool subset via `_PHASE_TOOLS` in `agent.py`.
...
- Import phase modules lazily inside command handlers (`from .phases.foo import run_foo`).

## FastMCP Tool Patterns
- Tools are decorated with `@mcp.tool()` on the server module.
- Tool handlers receive plain Python arguments; return plain strings.
- Phase tool subsets are defined in `_PHASE_TOOLS` in `agent.py` as `set[str]`.
...
## AgentLoop Patterns
- Always set `max_tokens` via `get_max_tokens(model.model_type, phase)` — never hardcode.
...
## Prompts and Skills
- Prompts live in `resources/prompts/<phase>.md` as named `##` sections.
- Load via `get_prompt("phase", "SECTION")` through the MCP tool handler.
