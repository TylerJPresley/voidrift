...
**Flash is the entry point.** Every user turn goes to the Flash model. It does the work.

**Utility is a helper.** It runs file summarization, indexing, and internal processing. It never talks to the user directly.
...
**Auto routing** adapts to the agent type. Interactive agents (Chat, Vibe) start on Flash. Task agents (indexer, summarizer, plugin agents) start on Utility. Both escalate to Dense when needed. Set `modelTier: "auto"` on any agent to get this behavior — or pin a specific tier to override.
...
**Message decay** proactively compacts old turns before they become a problem. After a configurable number of turns (default 20), older messages are automatically summarized — you don't have to wait for context to fill up. Set `decayAfterTurns` to 0 to disable.
...
| `decayAfterTurns` | 20 | Auto-compact messages older than N turns. 0 = no decay |
